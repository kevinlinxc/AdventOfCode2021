...
# count how many flashing octopi. Check when all of them flash at the same time for part 2.
...
def flash(row_index, col_index, rows):
    for delta in deltas:
    ...
    while any([value > 9 for value in [item for sublist in rows for item in sublist]]):
        for row_index, row in enumerate(rows):
            for col_index, num in enumerate(row):
                if num > 9:
                    flash(row_index, col_index, rows)
                    counter += 1
